...
version (unittest)
{
    ...
    string[] UNIT_TEST_DROP_TABLES_SCRIPT = [
        "DROP TABLE IF EXISTS role_users", "DROP TABLE IF EXISTS account_type",
        "DROP TABLE IF EXISTS users", "DROP TABLE IF EXISTS customers",
        "DROP TABLE IF EXISTS person", "DROP TABLE IF EXISTS person_info",
        "DROP TABLE IF EXISTS person_info2", "DROP TABLE IF EXISTS role",
    ...
    immutable bool DB_TESTS_ENABLED = SQLITE_TESTS_ENABLED || PGSQL_TESTS_ENABLED;
...
unittest
{
    ...
    assert(
            schema["Customer"]["address"].referencedEntity["streetAddress"].columnName
            == "street_address");
...
unittest
{
    ...
    static assert(getPropertyEmbeddedClassName!(EMUser, "userName") == "test.tests.EMName");
    ...
    static assert(getPropertyReferencedClassName!(Person, "moreInfo") == "test.tests.MoreInfo");
    ...
    //  }
    schema = new SchemaInfoImpl!(test.tests); //Person, MoreInfo, EvenMoreInfo, 
