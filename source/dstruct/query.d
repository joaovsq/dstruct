...
class FromClauseItem
{
	...
	this(const EntityInfo entity, string entityAlias, JoinType joinType, bool fetch,
			FromClauseItem base = null, const PropertyInfo baseProperty = null)
	{
...
class FromClause
{
	...
	FromClauseItem add(const EntityInfo entity, string entityAlias, JoinType joinType,
			bool fetch, FromClauseItem base = null, const PropertyInfo baseProperty = null)
	{
		FromClauseItem item = new FromClauseItem(entity, entityAlias, joinType,
				fetch, base, baseProperty);
	...
	{
		enforceHelper!DStructException(index >= 0 && index < items.length,
				"FromClause index out of range: " ~ to!string(index));
		return items[index];
	}
...
class QueryParser
{
	...
	void parse()
	{
		...
		enforceHelper!QuerySyntaxException(selectPos <= 0,
				"SELECT clause should be first - invalid query " ~ query);
		enforceHelper!QuerySyntaxException(wherePos == -1 || wherePos > fromPos,
				"Invalid WHERE position in query " ~ query);
		enforceHelper!QuerySyntaxException(orderPos == -1 || (orderPos < tokens.length - 2
				&& tokens[orderPos + 1].keyword == KeywordType.BY),
				"Invalid ORDER BY in query " ~ query);
		enforceHelper!QuerySyntaxException(orderPos == -1 || orderPos > fromPos,
				"Invalid ORDER BY position in query " ~ query);
		int fromEnd = len;
		if (orderPos >= 0)
			fromEnd = orderPos;
		if (wherePos >= 0)
	...
	private FromClauseItem ensureItemFetched(FromClauseItem a, const PropertyInfo p)
	{
		...
		if (res is null)
		{
			...
			res = fromClause.add(p.referencedEntity, null, p.nullable
					? JoinType.LeftJoin : JoinType.InnerJoin, true, a, p);
		}
		else
		{
			// force fetch
	...
	private void splitCommaDelimitedList(int start, int end, void delegate(int, int) callback)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			{
				enforceHelper!QuerySyntaxException(tokens[i].type != TokenType.Comma
						|| i != end - 1,
						"Invalid comma at end of list" ~ errorContext(tokens[start]));
				int endp = i < end - 1 ? i : end;
				enforceHelper!QuerySyntaxException(endp > p,
						"Invalid comma delimited list" ~ errorContext(tokens[start]));
				callback(p, endp);
				p = i + 1;
			}
		}
	...
	private int parseFieldRef(int start, int end, ref string[] path)
	{
		...
		while (pos < end)
		{
			...
			{
				enforceHelper!QuerySyntaxException(path.length == 0 || tokens[pos].type != TokenType.Alias,
						"Alias is allowed only as first item" ~ errorContext(tokens[pos]));
		...
		enforceHelper!QuerySyntaxException(tokens[pos].type != TokenType.Dot,
				"Unexpected dot at end in field list" ~ errorContext(tokens[pos]));
		enforceHelper!QuerySyntaxException(path.length > 0,
				"Empty field list" ~ errorContext(tokens[pos]));
		return pos;
	}

	private void parseFirstFromClause(int start, int end, out int pos)
	{
		enforceHelper!QuerySyntaxException(start < end,
				"Invalid FROM clause " ~ errorContext(tokens[start]));
		// minimal support:
		//    Entity
		//    Entity alias
		//    Entity AS alias
		enforceHelper!QuerySyntaxException(tokens[start].type == TokenType.Ident,
				"Entity name identifier expected in FROM clause" ~ errorContext(tokens[start]));
		...
		if (p < end)
		{
			enforceHelper!QuerySyntaxException(tokens[p].type == TokenType.Ident,
					"Alias name identifier expected in FROM clause" ~ errorContext(tokens[p]));
	...
	void appendFromClause(Token context, string[] path, string aliasName,
			JoinType joinType, bool fetch)
	{
		int p = 0;
		enforceHelper!QuerySyntaxException(fromClause.hasAlias(path[p]),
				"Unknown alias " ~ path[p] ~ " in FROM clause" ~ errorContext(context));
		...
		while (true)
		{
			...
			enforceHelper!QuerySyntaxException(p < path.length,
					"Property name expected in FROM clause" ~ errorContext(context));
			...
			enforceHelper!QuerySyntaxException(!property.simple,
					"Simple property " ~ propertyName ~ " cannot be used in JOIN" ~ errorContext(
						context));
			enforceHelper!QuerySyntaxException(!property.embedded,
					"Embedded property " ~ propertyName ~ " cannot be used in JOIN" ~ errorContext(
						context));
			bool last = (p == path.length);
			FromClauseItem item = fromClause.add(referencedEntity, last
					? aliasName : null, joinType, fetch, baseClause, property);
	...
	void parseFromClause(int start, int end)
	{
		...
		while (p < end)
		{
			...
			enforceHelper!QuerySyntaxException(p < end && tokens[p].keyword == KeywordType.JOIN,
					"Invalid FROM clause" ~ errorContext(tokens[p]));
			p++;
			enforceHelper!QuerySyntaxException(p < end,
					"Invalid FROM clause - incomplete JOIN" ~ errorContext(tokens[p]));
			bool fetch = false;
			...
			enforceHelper!QuerySyntaxException(p < end && tokens[p].type == TokenType.Ident,
					"Invalid FROM clause - no alias in JOIN" ~ errorContext(tokens[p]));
	...
	void addSelectClauseItem(string aliasName, string[] propertyNames)
	{
		...
		FromClauseItem from = aliasName == null ? fromClause.first
			: findFromClauseByAlias(aliasName);
		SelectClauseItem item;
		item.from = from;
		item.prop = null;
		...
		if (propertyNames.length > 0)
		{
			...
			while (item.prop.embedded)
			{
				...
				enforceHelper!QuerySyntaxException(propertyNames.length > 0,
						"@Embedded field property name should be specified when selecting "
						~ aliasName ~ "." ~ item.prop.propertyName);
				item.prop = cast(PropertyInfo) ei.findProperty(propertyNames[0]);
				propertyNames.popFront();
			}
		}
		enforceHelper!QuerySyntaxException(propertyNames.length == 0,
				"Extra field names in SELECT clause in query " ~ query);
		selectClause ~= item;
	...
	void parseOrderByClauseItem(int start, int end)
	{
		...
		else if (tokens[end - 1].type == TokenType.Keyword
				&& tokens[end - 1].keyword == KeywordType.DESC)
		{
			asc = false;
			end--;
		}
		enforceHelper!QuerySyntaxException(start < end,
				"Empty ORDER BY clause item" ~ errorContext(tokens[start]));
		if (start == end - 1)
		{
			// no alias
			enforceHelper!QuerySyntaxException(tokens[start].type == TokenType.Ident,
					"Property name expected in ORDER BY clause" ~ errorContext(tokens[start]));
			addOrderByClauseItem(null, cast(string) tokens[start].text, asc);
		}
		else if (start == end - 3)
		{
			enforceHelper!QuerySyntaxException(tokens[start].type == TokenType.Alias,
					"Entity alias expected in ORDER BY clause" ~ errorContext(tokens[start]));
			enforceHelper!QuerySyntaxException(tokens[start + 1].type == TokenType.Dot,
					"Dot expected after entity alias in ORDER BY clause" ~ errorContext(
						tokens[start]));
			enforceHelper!QuerySyntaxException(tokens[start + 2].type == TokenType.Ident,
					"Property name expected after entity alias in ORDER BY clause" ~ errorContext(
						tokens[start]));
			addOrderByClauseItem(cast(string) tokens[start].text,
					cast(string) tokens[start + 2].text, asc);
		}
		else
		{
			//writeln("range: " ~ to!string(start) ~ " .. " ~ to!string(end));
			enforceHelper!QuerySyntaxException(false,
					"Invalid ORDER BY clause (expected {property [ASC | DESC]} or {alias.property [ASC | DESC]} )"
					~ errorContext(tokens[start]));
		}
	}
	...
	void parseSelectClauseItem(int start, int end)
	{
		...
		enforceHelper!QuerySyntaxException(tokens[start].type == TokenType.Ident || tokens[start].type == TokenType.Alias,
				"Property name or alias expected in SELECT clause in query " ~ query ~ errorContext(
					tokens[start]));
		string aliasName;
		int p = start;
		if (tokens[p].type == TokenType.Alias)
		{
			...
			aliasName = cast(string) tokens[p].text;
			p++;
			enforceHelper!QuerySyntaxException(p == end || tokens[p].type == TokenType.Dot,
					"SELECT clause item is invalid (only  [alias.]field{[.field2]}+ allowed) " ~ errorContext(
						tokens[start]));
	...
	{
		enforceHelper!QuerySyntaxException(start < end,
				"Invalid SELECT clause" ~ errorContext(tokens[start]));
	...
	{
		enforceHelper!QuerySyntaxException(selectClause != null
				&& selectClause.length > 0, "Invalid SELECT clause");
		int aliasCount = 0;
		int fieldCount = 0;
		...
		enforceHelper!QuerySyntaxException((aliasCount == 1 && fieldCount == 0) || (aliasCount == 0
				&& fieldCount > 0), "You should either use single entity alias or one or more properties in SELECT clause. Don't mix objects with primitive fields");
		return aliasCount > 0;
	}
	...
	{
		enforceHelper!QuerySyntaxException(start < end,
				"Invalid WHERE clause" ~ errorContext(tokens[start]));
	...
	void foldBraces(ref Token[] items)
	{
		while (true)
		{
			...
			enforceHelper!QuerySyntaxException(lastOpen >= 0 && lastOpen < firstClose,
					"Unpaired braces in WHERE clause" ~ errorContext(tokens[lastOpen]));
			Token folded = new Token(items[lastOpen].pos, TokenType.Braces,
					items, lastOpen + 1, firstClose);
	...
	void convertIsNullIsNotNull(ref Token[] items)
	{
		...
		for (int i = cast(int) items.length - 3; i >= 0; i--)
		{
			if (items[i].type != TokenType.Operator
					|| items[i + 1].type != TokenType.Operator
					|| items[i + 2].type != TokenType.Keyword)
				continue;
			if (items[i].operator == OperatorType.IS
					&& items[i + 1].operator == OperatorType.NOT
					&& items[i + 2].keyword == KeywordType.NULL)
			{
		...
		while (true)
		{
			...
			for (int i = p + 1; i < items.length - 1; i += 2)
			{
				if (items[i].type != TokenType.Dot)
					break;
				enforceHelper!QuerySyntaxException(i < items.length - 1 && items[i + 1].type == TokenType.Ident,
						"Syntax error in WHERE condition - no property name after . " ~ errorContext(
							items[p]));
				lastp = i + 1;
			...
			enforceHelper!QuerySyntaxException(idents.length > 0,
					"Syntax error in WHERE condition - alias w/o property name: " ~ aliasName ~ errorContext(
						items[p]));
			PropertyInfo pi;
			fullName = aliasName;
			while (true)
			{
				...
				while (pi.embedded)
				{ // loop to allow nested @Embedded
					enforceHelper!QuerySyntaxException(idents.length > 0, "Syntax error in WHERE condition - @Embedded property reference should include reference to @Embeddable property " ~ aliasName ~ errorContext(
							items[p]));
				...
				if (idents.length > 0)
				{
					...
					enforceHelper!QuerySyntaxException(pi.referencedEntity !is null,
							"Unexpected extra field name " ~ pname ~ " - property " ~ propertyName
							~ " doesn't content subproperties " ~ errorContext(items[p]));
					...
					if (newClause is null)
					{
						...
						newClause = fromClause.add(ei, null, pi.nullable
								? JoinType.LeftJoin : JoinType.InnerJoin, false, a, pi);
					}
					a = newClause;
				}
			}
			enforceHelper!QuerySyntaxException(idents.length == 0,
					"Unexpected extra field name " ~ idents[0] ~ errorContext(items[p]));
	...
	static void convertUnaryPlusMinus(ref Token[] items)
	{
		...
		for (int i = 0; i < items.length; i++)
		{
			...
			if (op == OperatorType.ADD || op == OperatorType.SUB)
			{
				...
				{
					items[i].operator = (op == OperatorType.ADD)
						? OperatorType.UNARY_PLUS : OperatorType.UNARY_MINUS;
				}
			}
		}
	}
	...
	void foldCommaSeparatedList(Token braces)
	{
		...
		for (int i = 0; i <= items.length; i++)
		{
			...
			{
				enforceHelper!QuerySyntaxException(i > start,
						"Empty item in comma separated list" ~ errorContext(items[i]));
				enforceHelper!QuerySyntaxException(i != items.length - 1,
						"Empty item in comma separated list" ~ errorContext(items[i]));
				Token item = new Token(items[start].pos, TokenType.Expression,
						braces.children, start, i);
				foldOperators(item.children);
				enforceHelper!QuerySyntaxException(item.children.length == 1,
						"Invalid expression in list item" ~ errorContext(items[i]));
	...
	void foldOperators(ref Token[] items)
	{
		...
		while (true)
		{
			...
			if (t == OperatorType.NOT || t == OperatorType.UNARY_PLUS
					|| t == OperatorType.UNARY_MINUS)
			{
				// fold unary
				enforceHelper!QuerySyntaxException(bestOpPosition < items.length
						&& items[bestOpPosition + 1].isExpression(),
						"Syntax error in WHERE condition " ~ errorContext(items[bestOpPosition]));
				Token folded = new Token(items[bestOpPosition].pos, t,
						items[bestOpPosition].text, items[bestOpPosition + 1]);
				replaceInPlace(items, bestOpPosition, bestOpPosition + 2, [
						folded
						]);
			}
			...
			{
				// fold unary
				enforceHelper!QuerySyntaxException(bestOpPosition > 0 && items[bestOpPosition - 1].isExpression(),
						"Syntax error in WHERE condition " ~ errorContext(items[bestOpPosition]));
				Token folded = new Token(items[bestOpPosition - 1].pos, t,
						items[bestOpPosition].text, items[bestOpPosition - 1]);
				replaceInPlace(items, bestOpPosition - 1, bestOpPosition + 1, [
						folded
						]);
			}
			else if (t == OperatorType.BETWEEN)
			{
				...
				enforceHelper!QuerySyntaxException(bestOpPosition > 0,
						"Syntax error in WHERE condition - no left arg for BETWEEN operator");
				enforceHelper!QuerySyntaxException(bestOpPosition < items.length - 1,
						"Syntax error in WHERE condition - no min bound for BETWEEN operator " ~ errorContext(
							items[bestOpPosition]));
				enforceHelper!QuerySyntaxException(bestOpPosition < items.length - 3,
						"Syntax error in WHERE condition - no max bound for BETWEEN operator " ~ errorContext(
							items[bestOpPosition]));
				enforceHelper!QuerySyntaxException(items[bestOpPosition + 2].operator == OperatorType.AND,
						"Syntax error in WHERE condition - no max bound for BETWEEN operator" ~ errorContext(
							items[bestOpPosition]));
				Token folded = new Token(items[bestOpPosition - 1].pos, t,
						items[bestOpPosition].text, items[bestOpPosition - 1]);
				...
				replaceInPlace(items, bestOpPosition - 1, bestOpPosition + 4, [
						folded
						]);
			}
			else if (t == OperatorType.IN)
			{
				// fold  X IN (A, B, ...)
				enforceHelper!QuerySyntaxException(bestOpPosition > 0,
						"Syntax error in WHERE condition - no left arg for IN operator");
				enforceHelper!QuerySyntaxException(bestOpPosition < items.length - 1,
						"Syntax error in WHERE condition - no value list for IN operator " ~ errorContext(
							items[bestOpPosition]));
				enforceHelper!QuerySyntaxException(items[bestOpPosition + 1].type == TokenType.Braces,
						"Syntax error in WHERE condition - no value list in braces for IN operator" ~ errorContext(
							items[bestOpPosition]));
				Token folded = new Token(items[bestOpPosition - 1].pos, t,
						items[bestOpPosition].text, items[bestOpPosition - 1]);
				folded.children ~= items[bestOpPosition + 1];
				foldCommaSeparatedList(items[bestOpPosition + 1]);
				replaceInPlace(items, bestOpPosition - 1, bestOpPosition + 2, [
						folded
						]);
				// fold value list
			...
			else
			{
				// fold binary
				enforceHelper!QuerySyntaxException(bestOpPosition > 0,
						"Syntax error in WHERE condition - no left arg for binary operator " ~ errorContext(
							items[bestOpPosition]));
				enforceHelper!QuerySyntaxException(bestOpPosition < items.length - 1,
						"Syntax error in WHERE condition - no right arg for binary operator " ~ errorContext(
							items[bestOpPosition]));
				//writeln("binary op " ~ items[bestOpPosition - 1].toString() ~ " " ~ items[bestOpPosition].toString() ~ " " ~ items[bestOpPosition + 1].toString());
				enforceHelper!QuerySyntaxException(items[bestOpPosition - 1].isExpression(),
						"Syntax error in WHERE condition - wrong type of left arg for binary operator " ~ errorContext(
							items[bestOpPosition]));
				enforceHelper!QuerySyntaxException(items[bestOpPosition + 1].isExpression(),
						"Syntax error in WHERE condition - wrong type of right arg for binary operator " ~ errorContext(
							items[bestOpPosition]));
				Token folded = new Token(items[bestOpPosition - 1].pos, t,
						items[bestOpPosition].text, items[bestOpPosition - 1],
						items[bestOpPosition + 1]);
	...
	{
		enforceHelper!QuerySyntaxException(start < end,
				"Invalid ORDER BY clause" ~ errorContext(tokens[start]));
	...
	int addSelectSQL(Dialect dialect, ParsedQuery res, string tableName,
			bool first, const EntityInfo ei)
	{
		int colCount = 0;
		for (int j = 0; j < ei.getPropertyCount(); j++)
		{
			...
			if (f.embedded)
			{
				...
				colCount += addSelectSQL(dialect, res, tableName, first
						&& colCount == 0, f.referencedEntity);
				continue;
			}
			else if (f.oneToOne)
			{
	...
	void addFromSQL(Dialect dialect, ParsedQuery res)
	{
		...
		res.appendSQL(dialect.quoteIfNeeded(
				fromClause.first.entity.tableName) ~ " AS " ~ fromClause.first.sqlAlias);
		for (int i = 1; i < fromClause.length; i++)
		{
			...
			if (join.baseProperty.manyToMany)
			{
				...
				res.appendSQL(dialect.quoteIfNeeded(
						join.baseProperty.joinTable.tableName) ~ " AS " ~ joinTableAlias);
				res.appendSQL(" ON ");
			...
			else
			{
				...
				{
					assert(join.baseProperty.columnName !is null
							|| join.baseProperty.referencedProperty !is null);
					if (join.baseProperty.columnName !is null)
					{
						...
						res.appendSQL(join.sqlAlias);
						res.appendSQL(".");
						res.appendSQL(dialect.quoteIfNeeded(join.entity.getKeyProperty()
								.columnName));
					}
					else
					{
						...
						res.appendSQL(base.sqlAlias);
						res.appendSQL(".");
						res.appendSQL(dialect.quoteIfNeeded(base.entity.getKeyProperty()
								.columnName));
						res.appendSQL("=");
						res.appendSQL(join.sqlAlias);
						res.appendSQL(".");
						res.appendSQL(dialect.quoteIfNeeded(
								join.baseProperty.referencedProperty.columnName));
					}
				}
				else if (join.baseProperty.manyToOne)
				{
					assert(join.baseProperty.columnName !is null,
							"ManyToOne should have JoinColumn as well");
					//writeln("fk is in base");
					res.appendSQL(base.sqlAlias);
					res.appendSQL(".");
					res.appendSQL(dialect.quoteIfNeeded(join.baseProperty.columnName));
	...
	void addWhereCondition(Token t, int basePrecedency, Dialect dialect, ParsedQuery res)
	{
		...
		else if (t.type == TokenType.OpExpr)
		{
			...
			default:
				enforceHelper!QuerySyntaxException(false,
						"Unexpected operator" ~ errorContext(t));
				break;
			}
			if (needBraces)
				res.appendSQL(")");
...
KeywordType isKeyword(char[] str)
{
	char[] s = toUpper(str);
	if (s == "SELECT")
		return KeywordType.SELECT;
	if (s == "FROM")
		return KeywordType.FROM;
	if (s == "WHERE")
		return KeywordType.WHERE;
	if (s == "ORDER")
		return KeywordType.ORDER;
	if (s == "BY")
		return KeywordType.BY;
	if (s == "ASC")
		return KeywordType.ASC;
	if (s == "DESC")
		return KeywordType.DESC;
	if (s == "JOIN")
		return KeywordType.JOIN;
	if (s == "INNER")
		return KeywordType.INNER;
	if (s == "OUTER")
		return KeywordType.OUTER;
	if (s == "LEFT")
		return KeywordType.LEFT;
	if (s == "RIGHT")
		return KeywordType.RIGHT;
	if (s == "FETCH")
		return KeywordType.FETCH;
	if (s == "LIKE")
		return KeywordType.LIKE;
	if (s == "IN")
		return KeywordType.IN;
	if (s == "IS")
		return KeywordType.IS;
	if (s == "NOT")
		return KeywordType.NOT;
	if (s == "NULL")
		return KeywordType.NULL;
	if (s == "AS")
		return KeywordType.AS;
	if (s == "AND")
		return KeywordType.AND;
	if (s == "OR")
		return KeywordType.OR;
	if (s == "BETWEEN")
		return KeywordType.BETWEEN;
	if (s == "DIV")
		return KeywordType.DIV;
	if (s == "MOD")
		return KeywordType.MOD;
...
class Token
{
	...
	bool isExpression()
	{
		return type == TokenType.Expression || type == TokenType.Braces || type == TokenType.OpExpr
			|| type == TokenType.Parameter || type == TokenType.Field
			|| type == TokenType.String || type == TokenType.Number;
	}
	...
	override string toString()
	{
		...
		case TokenType.Field:
			return from.entityAlias ~ "." ~ field.propertyName; // field name of some entity
		case TokenType.Alias:
			return "alias: " ~ text; // alias name of some entity
		case TokenType.Parameter:
			return ":" ~ text; // ident after :
			// types of compound AST nodes
		case TokenType.Expression:
			return "expr"; // any expression
		case TokenType.Braces:
			return "()"; // ( tokens )
		case TokenType.CommaDelimitedList:
			return ",,,"; // tokens, ... , tokens
		case TokenType.OpExpr:
			return "" ~ text;
		default:
			return "UNKNOWN";
		}
...
Token[] tokenize(string s)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		else if (ch == ':' && (isAlpha(ch2) || ch2 == '_'))
		{
			...
			enforceHelper!QuerySyntaxException(text.length > 0,
					"Invalid parameter name near " ~ cast(string) s[startpos .. $]);
		...
		else if (isAlpha(ch) || ch == '_' || quotedIdent)
		{
			...
			{
				i++;
				enforceHelper!QuerySyntaxException(i < len - 1,
						"Invalid quoted identifier near " ~ cast(string) s[startpos .. $]);
			}
			// && state == 0
			for (int j = i; j < len; j++)
			{
			...
			if (quotedIdent)
			{
				enforceHelper!QuerySyntaxException(i < len - 1 && s[i + 1] == '`',
						"Invalid quoted identifier near " ~ cast(string) s[startpos .. $]);
				i++;
			}
		...
		else if (ch == '\'')
		{
			...
			enforceHelper!QuerySyntaxException(i < len - 1 && s[i + 1] == '\'',
					"Unfinished string near " ~ cast(string) s[startpos .. $]);
			i++;
		...
		else if (isDigit(ch) || (ch == '.' && isDigit(ch2)))
		{
			...
			if (i < len - 1 && std.ascii.toLower(s[i + 1]) == 'e')
			{
				...
				enforceHelper!QuerySyntaxException(i < len - 1 && isDigit(s[i]),
						"Invalid number near " ~ cast(string) s[startpos .. $]);
				for (int j = i; j < len; j++)
				{
					if (isDigit(s[j]))
					{
			...
			enforceHelper!QuerySyntaxException(i >= len - 1 || !isAlpha(s[i]),
					"Invalid number near " ~ cast(string) s[startpos .. $]);
		...
		{
			enforceHelper!QuerySyntaxException(false,
					"Invalid character near " ~ cast(string) s[startpos .. $]);
		}
	}
	return res;
}
...
class ParameterValues
{
	...
	{
		enforceHelper!QueryParameterException((name in params) !is null,
				"Attempting to set unknown parameter " ~ name);
...
class ParsedQuery
{
	...
	@property const(EntityInfo) entity()
	{
		return _entity;
	}
	...
	@property SelectClauseItem[] select()
	{
		return _select;
	}
...
unittest
{
	...
	EntityMetaData schema = new SchemaInfoImpl!(User, Customer, AccountType,
			Address, Person, MoreInfo, EvenMoreInfo, Role);
	...
	parser = new QueryParser(schema,
			"SELECT a2 FROM User AS a1 JOIN a1.roles AS a2 WHERE a1.id = 1");
	q = parser.makeSQL(dialect);
	//writeln(q.hql);
	//writeln(q.sql);
