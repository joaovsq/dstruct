...
class EntityCache
{
    ...
    {
        enforceHelper!CacheException((key in items) !is null,
                "entity " ~ name ~ " with key " ~ key.toString() ~ " not found in cache");
        return items[key];
    }
...
class SessionAccessor
{
    ...
    SessionImpl get()
    {
        enforceHelper!LazyInitializationException(_session !is null,
                "Cannot read from closed session");
        return _session;
    }
...
class SessionImpl : Session
{
    ...
    this(SessionFactoryImpl sessionFactory, EntityMetaData metaData,
            Dialect dialect, DataSource connectionPool)
    {
    ...
    override Object loadObject(string entityName, Variant id)
    {
        ...
        enforceHelper!ObjectNotFoundException(obj !is null,
                "Entity " ~ entityName ~ " with id " ~ to!string(id) ~ " not found");
        return obj;
    }
    ...
    override void loadObject(Object obj, Variant id)
    {
        ...
        enforceHelper!ObjectNotFoundException(found !is null,
                "Entity " ~ info.name ~ " with id " ~ to!string(id) ~ " not found");
    }
    ...
    {
        string hql = "SELECT a2 FROM " ~ info.name ~ " AS a1 JOIN a1." ~ referencePropertyName
            ~ " AS a2 WHERE a1." ~ info.getKeyProperty().propertyName ~ "=:Fk";
    ...
    override void refresh(Object obj)
    {
        ...
        enforceHelper!TransientObjectException(info.isKeySet(obj),
                "Cannot refresh entity " ~ info.name ~ ": no Id specified");
        ...
        {
            // not found!
            enforceHelper!ObjectNotFoundException(false,
                    "Entity " ~ info.name ~ " with id " ~ to!string(id) ~ " not found");
        }
    }
    ...
    private void updateRelations(const PropertyInfo p, Object obj)
    {
        ...
        if (keysToDelete.length > 0)
        {
            ...
            scope (exit)
                stmt.close();
            stmt.executeUpdate(p.joinTable.getDeleteSQL(dialect,
                    createKeySQL(thisId), keysToDelete));
        }
    }
    ...
    override Variant save(Object obj)
    {
        ...
        if (!info.isKeySet(obj))
        {
            if (info.getKeyProperty().generated)
            {
                ...
                string query = dialect.appendInsertToFetchGeneratedKey(
                        metaData.generateInsertNoKeyForEntity(dialect, info), info);
                PreparedStatement stmt = conn.prepareStatement(query);
                scope (exit)
                    stmt.close();
            ...
            else if (info.getKeyProperty().generatorFunc !is null)
            {
                ...
                Variant generatedKey = info.getKeyProperty()
                    .generatorFunc(conn, info.getKeyProperty());
                info.setKey(obj, generatedKey);
    ...
    override void persist(Object obj)
    {
        auto info = metaData.findEntityForObject(obj);
        enforceHelper!TransientObjectException(info.isKeySet(obj),
                "Cannot persist entity w/o key assigned");
        string query = metaData.generateInsertAllFieldsForEntity(dialect, info);
        PreparedStatement stmt = conn.prepareStatement(query);
        scope (exit)
            stmt.close();
    ...
    override void update(Object obj)
    {
        ...
        //writeln("Query: " ~ query);
        {
            ...
            scope (exit)
                stmt.close();
            const int columnCount = metaData.writeAllColumns(obj, stmt, 1, true);
    ...
    override void remove(Object obj)
    {
        ...
        string query = "DELETE FROM " ~ dialect.quoteIfNeeded(
                info.tableName) ~ " WHERE " ~ dialect.quoteIfNeeded(info.getKeyProperty()
                .columnName) ~ "=?";
        PreparedStatement stmt = conn.prepareStatement(query);
...
class PropertyLoadItem
{
    ...
    @property ref ObjectList[Variant] map()
    {
        return _map;
    }
...
class PropertyLoadMap
{
    ...
    @property ref PropertyLoadItem[const PropertyInfo] map()
    {
        return _map;
    }
    ...
    @property const(PropertyInfo)[] keys()
    {
        return _map.keys;
    }
...
class QueryImpl : Query
{
    ...
    override Object uniqueObject(Object obj)
    {
        ...
        if (rows == null)
            return null;
        enforceHelper!NonUniqueResultException(rows.length == 1,
                "Query returned more than one object: " ~ getQueryString());
        return rows[0];
    }
    ...
    override Variant[] uniqueRow()
    {
        ...
        if (rows == null)
            return null;
        enforceHelper!NonUniqueResultException(rows.length == 1,
                "Query returned more than one row: " ~ getQueryString());
        return rows[0];
    }
            ...
            for (int j = 0; j < ei.length; j++)
            {
                ...
                if (pi.oneToOne || pi.manyToOne)
                {
                    ...
                    else
                    {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (pi.lazyLoad)
                                    {
                                        // lazy load
                                        static if (TRACE_REFS)
                                            writeln("scheduling lazy load for " ~ from.pathString ~ "."
                                                    ~ pi.propertyName ~ " with FK " ~ id.toString);
                                        LazyObjectLoader loader = new LazyObjectLoader(sess.accessor,
                                                pi, id);
                                    ...
                                    else
                                    {
                                        // delayed load
                                        static if (TRACE_REFS)
                                            writeln("relation " ~ pi.propertyName ~ " with FK " ~ id.toString()
                        ...
                        else
                        {
                            // TODO:
                            assert(false,
                                    "relation " ~ pi.propertyName
                                    ~ " has no column name. To be implemented.");
                        }
                    }
                }
                else if (pi.oneToMany || pi.manyToMany)
                {
                    ...
                    if (pi.lazyLoad)
                    {
                        // lazy load
                        static if (TRACE_REFS)
                            writeln("creating lazy loader for " ~ from.pathString
                                    ~ "." ~ pi.propertyName ~ " by FK " ~ id.toString);
                        LazyCollectionLoader loader = new LazyCollectionLoader(sess.accessor,
                                pi, id);
                    ...
                    else
                    {
                        // delayed load
                        static if (TRACE_REFS)
                            writeln("Relation " ~ from.pathString ~ "." ~ pi.propertyName
                                    ~ " will be loaded later by FK " ~ id.toString);
                        loadMap.add(pi, id, relations[i]); // to load later
                    }
                }
            }
    ...
    private void delayedLoadRelations(PropertyLoadMap loadMap)
    {
        ...
        foreach (pi; types)
        {
            ...
            if (pi.oneToOne || pi.manyToOne)
            {
                ...
                {
                    Variant[] unknownKeys;
                    Object[] list = sess.lookupCache(pi.referencedEntity.name,
                            map.keys, unknownKeys);
                    if (unknownKeys.length > 0)
                    {
                        string hql = "FROM " ~ pi.referencedEntity.name ~ " WHERE "
                            ~ pi.referencedEntity.keyProperty.propertyName ~ " IN (" ~ createCommaSeparatedKeyList(
                                    unknownKeys) ~ ")";
                        static if (TRACE_REFS)
                            writeln(
                                    "delayedLoadRelations: loading "
                                    ~ pi.propertyName ~ " HQL: " ~ hql);
                        QueryImpl q = cast(QueryImpl) sess.createQuery(hql);
                        Object[] fromDB = q.listObjects(null, loadMap);
                        list ~= fromDB;
                        static if (TRACE_REFS)
                            writeln("delayedLoadRelations: objects loaded " ~ to!string(
                                    fromDB.length));
                    }
                    else
                    {
                    ...
                    foreach (rel; list)
                    {
                        static if (TRACE_REFS)
                            writeln(
                                    "delayedLoadRelations: reading key from "
                                    ~ pi.referencedEntity.name);
                ...
                {
                    assert(false,
                            "Delayed loader for non-join column is not yet implemented for OneToOne and ManyToOne");
                }
            }
            else if (pi.oneToMany || pi.manyToMany)
            {
                string hql = "FROM " ~ pi.referencedEntity.name ~ " WHERE " ~ pi.referencedPropertyName ~ "."
                    ~ pi.referencedEntity.keyProperty.propertyName ~ " IN (" ~ keys ~ ")";
                static if (TRACE_REFS)
                ...
                static if (TRACE_REFS)
                    writeln("delayedLoadRelations oneToMany/manyToMany: objects loaded " ~ to!string(
                            list.length));
                ...
                {
                    static if (TRACE_REFS)
                        writeln("delayedLoadRelations oneToMany/manyToMany: reading reference from "
                                ~ pi.referencedEntity.name ~ "." ~ pi.referencedProperty.propertyName
                                ~ " joinColumn=" ~ pi.referencedProperty.columnName);
                    assert(pi.referencedProperty.manyToOne,
                            "property referenced from OneToMany should be ManyToOne");
    ...
    Object[] listObjects(Object placeFirstObjectHere, PropertyLoadMap loadMap)
    {
        ...
        enforceHelper!SessionException(ei !is null,
                "No entity expected in result of query " ~ getQueryString());
        ...
        // const int startColumn = query.select[0].from.startColumn;
        {
            scope (exit)
                rs.close();
            while (rs.next())
            {
                Object row = readRelations(res.length > 0 ? null : placeFirstObjectHere,
                        rs, loadMap);
                if (row !is null)
                    res ~= row;
            }
        ...
        {
            static if (TRACE_REFS)
                writeln("relation properties scheduled for load: loadMap.length == " ~ to!string(
                        loadMap.length));
...
class LazyObjectLoader
{
    ...
    {
        static if (TRACE_REFS)
            writeln("Created lazy loader for " ~ pi.referencedEntityName ~ " with id " ~ id
                    .toString);
        this.pi = pi;
        this.id = id;
        this.sess = sess;
...
class LazyCollectionLoader
{
    ...
    {
        assert(!pi.oneToMany || (pi.referencedEntity !is null && pi.referencedProperty !is null),
                "LazyCollectionLoader: No referenced property specified for OneToMany foreign key column");
        static if (TRACE_REFS)
            writeln("Created lazy loader for collection for references "
                    ~ pi.entity.name ~ "." ~ pi.propertyName ~ " by id " ~ fk.toString);
        this.pi = pi;
        this.fk = fk;
        this.sess = sess;
    }
    ...
    Object[] load()
    {
        ...
        static if (TRACE_REFS)
            writeln(
                    "lazy loading of references " ~ pi.entity.name ~ "."
                    ~ pi.propertyName ~ " by id " ~ fk.toString);
